...
NumOfNodes = str2num(Val(nodes_t('1,','1,')));
     ...
     if(~exist([root '/mydata' num2str(NumOfNodes)],'dir'))
        mkdir([root '/mydata' num2str(NumOfNodes)]);
     else
	rmdir([root '/mydata' num2str(NumOfNodes)],'s');
	mkdir([root '/mydata' num2str(NumOfNodes)]);
        end
    if(~exist([root '/storelog' num2str(NumOfNodes)],'dir'))
        mkdir([root '/storelog' num2str(NumOfNodes)]);
    else
	rmdir([root '/storelog' num2str(NumOfNodes)],'s');
	mkdir([root '/storelog' num2str(NumOfNodes)]);
        end
...
for i = myProc
	 flog = fopen([root '/storelog' num2str(NumOfNodes)  '/' num2str(i) '.txt'],'w');
	...
	fwrite(flog, ['columns is ' num2str(columns)]);
		...
		[mr,mc,maxCol] = num(sprintf('%d,',columns),:);
			...
			fid = fopen([root '/mydata' num2str(NumOfNodes)  '/' num2str(i) '.txt'],'a');
	...
	disp(['I am just waiting!']);
     ...
     fid = fopen([root '/mydata' num2str(NumOfNodes) '/done.txt'],'w');
     fwrite(fid, '** Done**');
     fclose(fid);
     fclose(flog);
agg(w);
